...
instance Printable ResourceMarket where
  prettyPrint rm =
    ...
    where
      labels = ["Coal: ", "Garbage: ", "Oil: ", "Uranum: "]
...
instance Printable (Map.Map Name Balance) where
  prettyPrint = unwords . fmap printEntry . Map.toList
    where
      printEntry (name, bal) = name ++ ": " ++ show bal

instance Printable Game where
  prettyPrint game = unlines [printPart phase, printPart resourceMarket, printPart accounts]
    where
      printPart :: (Printable p) => Getting p Game p -> String
      printPart = prettyPrint . flip view game

main :: IO ()
main = do
  accounts <- setUp
  if Map.size accounts > 1
    then evalStateT (forever run) $ initialGame accounts
    else putStrLn "Not enough players"
...
chooseName names =
  ...
  where
    errMsg = "Player must be one of: " ++ unwords names
...
setUp = do
  ...
  where
    buildMap = foldr (\k -> Map.insert k 0) Map.empty
